...
void inline_parse(const char* in, char* out, size_t out_size) {
  ...
  while (in[i] && j < out_size - 1) {
    ...
    if (in[i] == '[') {
      ...
      size_t text_len        = 0;
      while (in[link_text_start + text_len] && in[link_text_start + text_len] != ']')
        text_len++;
      if (in[link_text_start + text_len] == ']') {
        if (in[link_text_start + text_len + 1] == '(') {
          ...
          if (in[url_start + url_len] == ')') {
            ...
            snprintf(link_html, sizeof(link_html), "<a href=\"%.*s\">%.*s</a>", (int) url_len,
                     in + url_start, (int) text_len, in + link_text_start);
...
void block_parse(const char* input, char* output, size_t out_size) {
  ...
  while (*p) {
    ...
    if (line[0] == '>') {
      ...
      snprintf(blockquote_html, sizeof(blockquote_html), "<blockquote>%s</blockquote>\n", line + 2);
...
void md_to_html(const char* markdown, char* html, size_t html_size, const char* css_file) {
  ...
  if (css_file && strlen(css_file) > 0) {
    snprintf(html + strlen(html), html_size - strlen(html), "<link rel=\"stylesheet\" href=\"%s\">",
             css_file);
  }
